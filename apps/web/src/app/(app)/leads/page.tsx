  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900">Funil de Leads</h1>
          <p className="text-slate-500 mt-1">Transforme interessados em clientes ativos da sua oficina.</p>
        </div>
        
        <div className="flex items-center gap-2">
            <div className="px-4 py-2 bg-white border border-slate-200 rounded-xl shadow-sm text-sm font-bold flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 border border-slate-200 bg-white p-8 rounded-[32px] shadow-xl shadow-slate-100 h-fit">
          <div className="flex items-center gap-3 mb-8">
            <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-blue-200">
              <UserPlus className="w-6 h-6" />
            </div>
            <div>
                <h2 className="text-lg font-black text-slate-800">Captura</h2>
                <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Entrada Manual</p>
            </div>
          </div>
          ...
          <form className="space-y-5" onSubmit={handleCreate}>
            ...
            <button 
                disabled={isCreating} 
                type="submit" 
                className="w-full group relative overflow-hidden bg-slate-900 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50"
            >
        ...
        <div className="lg:col-span-3 space-y-4">
            <div className="flex items-center justify-between mb-2">
                ...
                <div className="relative">
                    ...
                    <input type="text" placeholder="Filtrar leads..." className="pl-10 pr-4 py-2 border border-slate-200 rounded-xl text-xs outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {isLoading && Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="h-32 bg-slate-50 rounded-[28px] animate-pulse border border-slate-100" />
                ))}

                {!isLoading && leads.length === 0 && (
                    <div className="col-span-full py-20 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[32px] text-center">
                        <Target className="w-12 h-12 mx-auto mb-4 text-slate-200" />
                        <p className="font-bold text-slate-400 uppercase tracking-widest text-xs">Aguardando primeiros leads...</p>
                    </div>
                )}
                
                {leads.map((lead) => (
                    <div key={lead.id} className="bg-white border border-slate-200 p-6 rounded-[32px] shadow-sm hover:shadow-xl hover:border-blue-200 transition-all group overflow-hidden relative">
                    <div className="flex items-start justify-between relative z-10">
                        ...
                        <p className="font-black text-slate-900 text-lg tracking-tight">{lead.nome}</p>
                        <p className="text-slate-500 font-mono text-xs">{lead.telefone}</p>
                        </div>
                        ...
                        {lead.status !== 'convertido' ? (
                            <button 
                                ...
                                className="bg-emerald-500 hover:bg-emerald-600 text-white p-3 rounded-2xl shadow-lg shadow-emerald-100 transition-all hover:scale-110 active:scale-90"
                    ...
                    <div className="mt-6 pt-6 border-t border-slate-50 flex items-center justify-between">
                        <span className="text-[10px] font-bold text-slate-300 uppercase">Entrou em {new Date(lead.created_at).toLocaleDateString()}</span>
                        {lead.status === 'convertido' && (
                            <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Atendimento Ativo</span>
                        )}
                    </div>

                    {/* Background Decorative Element */}
                    <div className="absolute -right-4 -bottom-4 w-24 h-24 bg-slate-50 rounded-full opacity-0 group-hover:opacity-100 transition-all scale-150" />
                    </div>
                ))}
            </div>
        </div>
